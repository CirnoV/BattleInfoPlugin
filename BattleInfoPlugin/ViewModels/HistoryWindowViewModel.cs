			...
			if (this.BattleCalculator != null)
			{
				try
				{
					...
					foreach (var log in logs)
					{
						...
						if (aliasLogs.Contains(log)) // 아군이 맞은 피해 (index: 적군, target: 아군)
						{
							...
							else
							{
								if (log.Index >= 0)
									fromShip = fromCombined
										? this.BattleCalculator.EnemySecondShips[log.Index - 6]
							...
							if (log.Target >= 0)
								targetShip = targetCombined
									? this.BattleCalculator.AliasSecondShips[log.Target - 6]
									: this.BattleCalculator.AliasFirstShips[log.Target];
							else
								targetShip = null;
						}
						else // 아군이 준 피해 (index: 아군, target: 적군)
						{
